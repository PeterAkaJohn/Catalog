...
def fbconnect():
    if request.args.get('state') != login_session['state']:
        ...
        response.headers['Content-Type'] = 'authlication/json'
        return response
    ...
    # see if user exists
    user_id = Util.getUserID(login_session['email'])
    if not user_id:
        user_id = Util.createUser(login_session)
...
def gconnect():
    ...
    # Verify that the access token is valid for this auth.
    if result['issued_to'] != CLIENT_ID:
        response = make_response(
            json.dumps("Token's client ID does not match auth's."), 401)
        print "Token's client ID does not match auth's."
        response.headers['Content-Type'] = 'authlication/json'
        return response
...
@auth.route('/gconnect2', methods=['POST'])
def gconnect2():
    # Validate state token
    if request.args.get('state') != login_session['state']:
        response = make_response(json.dumps('Invalid state parameter.'), 401)
    ...
    login_session['gplus_id'] = code['id']
    login_session['username'] = code['username']
    login_session['picture'] = code['picture']
    login_session['email'] = code['email']
    # ADD PROVIDER TO LOGIN SESSION
    login_session['provider'] = 'google'
